...
class DistanceConstraint(Loss_ABC):
    ...
    def forward(
        ...
        # Get restype tensor for residue type lookup
        restype = trunk_input["f"]["restype"]
        ...
        for i, constraint in enumerate(self.constraints):
            ...
            if len(tok1_loc) == 0 or len(tok2_loc) == 0:
                ...
                # Get residue types (convert from one-hot restype to amino acid codes)
                # restype is one-hot encoded [32], use argmax to get the index
                res1_onehot = restype[tok1_loc[0]]  # [32] one-hot vector
                res2_onehot = restype[tok2_loc[0]]  # [32] one-hot vector

                # Get the index from argmax of the protein part (first 20 dimensions)
                res1_idx = torch.argmax(res1_onehot[:20]).item()
                res2_idx = torch.argmax(res2_onehot[:20]).item()

                # AF3 residue type mapping (standard 20 amino acids)
                aa_codes = [
                    "A",
                    "R",
                    "N",
                ...
                res1_type = (
                    aa_codes[res1_idx] if 0 <= res1_idx < 20 else f"IDX_{res1_idx}"
                )
                res2_type = (
                    aa_codes[res2_idx] if 0 <= res2_idx < 20 else f"IDX_{res2_idx}"
                )

                # Print debugging info
                print(
                    f"Distance constraint {i + 1}: Entity{entity1} Res{res1}({res1_type}) ↔ Entity{entity2} Res{res2}({res2_type}) = {distance.detach().item():.3f}Å"
                )
